...
def contract_vertex_and_copy(graph, u, v):
    ...
    for _, w, d in graph.edges(v, data=True):
        if w == u:
            continue
        elif (u, w) in new_graph.edges(u):
            new_graph[u][w]['capacity'] += d['capacity']
        else:
            new_graph.add_edge(u, w, capacity=d['capacity'])
    # remove node
    new_graph.remove_node(v)
    # keep a record of all contracted nodes
    if 'combined' in new_graph.node[u]:
        new_graph.node[u]['combined'].add(v)
    else:
        new_graph.node[u]['combined'] = {v}
    return new_graph
